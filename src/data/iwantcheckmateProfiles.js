const VIDEO_DIALOGUE_POLICY = 'iwantcheckmate'

function videoVariant({
  id,
  name,
  rating = null,
  country = 'United States',
  countryCode = 'us',
  category = null,
  avatar,
  avatarScale = null,
  avatarStates = null,
  identityStates = null,
  description,
  videoId,
  videoTitle,
  portraitSourceUrl = null,
  initialElo = 3600,
  minElo = 250,
  maxElo = 3600,
  trigger = null,
  eloDelta = 0,
  movePolicy = { type: 'best' },
  capabilities = {},
}) {
  const resolvedCategory = category
    || (id === 'iwc-smartin' || id.includes('martin') ? 'martin' : 'stockfish')
  const defaultAvatar = imageAvatar(
    name,
    avatar,
    avatarScale ?? 1,
  )
  const resolvedIdentityStates = identityStates
    ? Object.freeze(Object.fromEntries(
        Object.entries(identityStates).map(([state, identity]) => [
          state,
          Object.freeze({ ...identity }),
        ]),
      ))
    : null
  const resolvedAvatarStates = avatarStates
    ? Object.freeze(Object.fromEntries(
        Object.entries(avatarStates).map(([state, filename]) => [
          state,
          imageAvatar(resolvedIdentityStates?.[state]?.name || name, filename),
        ]),
      ))
    : null
  return Object.freeze({
    id,
    name,
    fullName: name,
    displayRating: rating,
    title: '',
    country,
    countryCode,
    category: resolvedCategory,
    avatar: defaultAvatar,
    avatarStates: resolvedAvatarStates,
    identityStates: resolvedIdentityStates,
    accounts: Object.freeze({ chesscom: [], lichess: [] }),
    intro: description,
    videoLabel: description,
    dialoguePolicy: VIDEO_DIALOGUE_POLICY,
    capabilities: Object.freeze({
      silentDialogue: false,
      videoVariant: true,
      beltMode: false,
      knightSpecialist: false,
      perfectTheory: false,
      maximumEngine: true,
      bishopKnightObjective: false,
      exactTablebase: false,
      ...capabilities,
    }),
    strengthPolicy: Object.freeze({
      engineElo: null,
      depth: 17,
      moveTime: 1150,
      candidates: 16,
      styleWindowCp: 0,
      bookWindowCp: 0,
      bookMinGames: Number.POSITIVE_INFINITY,
      bookMinRecentWeight: Number.POSITIVE_INFINITY,
      mateSafety: resolvedCategory === 'martin'
        ? Object.freeze({
            depth: 20,
            moveTime: 900,
            candidates: 1,
          })
        : null,
    }),
    repertoireSource: Object.freeze({
      chesscom: [],
      lichess: [],
      recentHalfLifeDays: 180,
    }),
    source: Object.freeze({
      channel: 'IWantCheckmate',
      videoId,
      videoTitle,
      videoUrl: videoId
        ? `https://www.youtube.com/watch?v=${videoId}`
        : 'https://iwantcheckmate.com/',
      portraitSourceUrl,
    }),
    variant: Object.freeze({
      initialElo,
      minElo,
      maxElo,
      trigger,
      eloDelta,
      movePolicy: Object.freeze(movePolicy),
    }),
  })
}

function imageAvatar(name, filename, scale = 1) {
  return Object.freeze({
    type: 'image',
    src: `./assets/iwantcheckmate/${filename}`,
    alt: `${name} in-video profile`,
    objectPosition: '50% 50%',
    scale,
    transparent: true,
  })
}

export const IWANTCHECKMATE_VIDEO_PROFILES = Object.freeze([
  videoVariant({
    id: 'iwc-worst-move',
    name: 'PityFish',
    rating: 3600,
    avatar: 'pityfish-profile.png',
    description: 'Loses 500 Elo when you find the true worst move.',
    videoId: 'SbPYufgaG-I',
    videoTitle: 'Stockfish, But It Loses 500 ELO If I Play the WORST Move',
    trigger: 'opponent-worst-move',
    eloDelta: -500,
    minElo: 100,
  }),
  videoVariant({
    id: 'iwc-give-check',
    name: 'PanicFish',
    rating: 3600,
    avatar: 'panicfish-profile.png',
    description: 'Loses 300 Elo whenever you give check.',
    videoId: 'vIvVdaTQi3s',
    videoTitle: 'Stockfish, But It Loses 300 ELO When I Give Check',
    trigger: 'opponent-check',
    eloDelta: -300,
    minElo: 100,
  }),
  videoVariant({
    id: 'iwc-best-move',
    name: 'TiltFish',
    rating: 3600,
    avatar: 'tiltfish-profile.png',
    description: 'Loses 100 Elo whenever you play Stockfish’s best move.',
    videoId: 'U6J7XjR_5Ik',
    videoTitle: 'Stockfish, But It Loses 100 ELO If I Play The BEST Move',
    trigger: 'opponent-best-move',
    eloDelta: -100,
    minElo: 100,
  }),
  videoVariant({
    id: 'iwc-smartin',
    name: 'Smartin',
    rating: 250,
    avatar: 'smartin-profile.png',
    description: 'Starts at 250 and gains 100 Elo after every move.',
    videoId: 'PPvPTwZg0JQ',
    videoTitle: 'Martin, But He Gains 100 ELO Every Move',
    initialElo: 250,
    trigger: 'own-move',
    eloDelta: 100,
    movePolicy: { type: 'rating-strength' },
  }),
  videoVariant({
    id: 'iwc-best-move-martin',
    name: 'Martin',
    rating: 250,
    country: '',
    countryCode: '',
    category: 'martin',
    avatar: 'martin-profile.png',
    description: 'Starts at 250 Elo and gains 200 Elo every time you do not play the best move.',
    videoId: 'nx6jkHl2k_Y',
    videoTitle: 'Martin, But He Gains 200 ELO If I DON\'T Make the Best Move',
    initialElo: 250,
    minElo: 250,
    maxElo: 3600,
    trigger: 'opponent-non-best-move',
    eloDelta: 200,
    movePolicy: {
      type: 'rating-strength',
      bestMoveContract: 'centipawn-tolerance',
      bestMoveToleranceCp: 20,
      ruleStatus: 'implemented',
      unverifiedAssumptions: ['3600-elo-cap'],
    },
  }),
  videoVariant({
    id: 'iwc-elo-decay',
    name: 'TiredFish',
    rating: 3600,
    avatar: 'tiredfish-profile.png',
    description: 'Starts at 3600 and loses 50 Elo after every move.',
    videoId: 'Uccowk6xbFc',
    videoTitle: 'Stockfish, But It Loses 50 ELO Every Move',
    trigger: 'own-move',
    eloDelta: -50,
    minElo: 100,
    movePolicy: { type: 'rating-strength' },
  }),
  videoVariant({
    id: 'iwc-random-blunder',
    name: 'BlunderFish',
    avatar: 'blunderfish-profile.png',
    description: 'Plays full strength, with a 5% chance to make a real blunder.',
    videoId: 'ROkJSsMOfu0',
    videoTitle: 'Stockfish, But 5% Chance of Blundering Every Move',
    movePolicy: { type: 'random-blunder', chance: 0.05, minimumLossCp: 200 },
  }),
  videoVariant({
    id: 'geometricfish',
    name: 'GeometricFish',
    rating: 3600,
    category: 'stockfish',
    avatar: 'geometricfish-profile.jpeg',
    description: 'Chooses among its strongest ideas in a geometric pattern.',
    videoId: 'Y0zYxMYDDWQ',
    videoTitle: 'Stockfish, But It Plays nth Best Move Where P(n) = (1/2)^n',
    movePolicy: { type: 'geometric-ranked', count: 6, firstWeight: 0.5 },
  }),
  videoVariant({
    id: 'iwc-clockfish',
    name: 'ClockFish',
    rating: 100,
    avatar: 'clockfish-profile.jpeg',
    description: 'Starts at 100 Elo and climbs to 3600 over five seconds of your turn.',
    videoId: '0rlkE-CYRoc',
    videoTitle: 'Stockfish, But It Gains 3600 ELO in 5 Seconds…',
    portraitSourceUrl: 'https://iwantcheckmate.com/bot-images/clockfish.jpeg',
    initialElo: 100,
    minElo: 100,
    maxElo: 3600,
    movePolicy: {
      type: 'time-ramp-strength',
      startElo: 100,
      endElo: 3600,
      durationMs: 5000,
    },
  }),
  videoVariant({
    id: 'iwc-mirrorfish',
    name: 'MirrorFish',
    avatar: 'mirrorfish-profile.jpeg',
    description: 'Matches the engine rank of your last move.',
    videoId: 'eCC-U9w9RvQ',
    videoTitle: 'Stockfish, But It Copies Your Move Quality…',
    portraitSourceUrl: 'https://iwantcheckmate.com/bot-images/mirrorfish.jpeg',
    movePolicy: { type: 'mirror-rank' },
  }),
  videoVariant({
    id: 'iwc-zebrafish',
    name: 'ZebraFish',
    avatar: 'zebrafish-profile.jpeg',
    description: 'Alternates between landing on light and dark squares.',
    videoId: 'oJu13RUrGl4',
    videoTitle: 'Stockfish, But It Alternates Playing Light and Dark Squares.',
    portraitSourceUrl: 'https://iwantcheckmate.com/bot-images/zebrafish.jpeg',
    movePolicy: { type: 'alternating-square', startsOn: 'light', allLegalMoves: true },
  }),
  videoVariant({
    id: 'iwc-simpfish',
    name: 'SimpFish',
    avatar: 'simpfish-profile.jpeg',
    description: 'Hunts your queen before it turns toward the king.',
    videoId: 'My1sUnkiNfU',
    videoTitle: 'Stockfish, But It Wants to Capture Your Queen...',
    portraitSourceUrl: 'https://iwantcheckmate.com/bot-images/simpfish.jpeg',
    movePolicy: { type: 'queen-hunter', allLegalMoves: true },
  }),
  videoVariant({
    id: 'iwc-checkfish',
    name: 'CheckFish',
    avatar: 'checkfish-profile.jpeg',
    description: 'If a legal check exists, it has to play one.',
    videoId: 'tNgzqWiYDFk',
    videoTitle: 'Stockfish, But If It Can Give Check, It Must',
    portraitSourceUrl: 'https://iwantcheckmate.com/bot-images/checkfish.jpeg',
    movePolicy: { type: 'forcing-check', allLegalMoves: true },
  }),
  videoVariant({
    id: 'iwc-scaredfish',
    name: 'ScaredFish',
    avatar: 'scaredfish-profile.jpeg',
    description: 'Treats every enemy pawn like a queen.',
    videoId: null,
    videoTitle: 'Stockfish, But It Thinks Your Pawns are Queens',
    portraitSourceUrl: 'https://iwantcheckmate.com/bot-images/scaredfish.jpeg',
    movePolicy: { type: 'pawn-as-queen', allLegalMoves: true },
  }),
  videoVariant({
    id: 'iwc-random-top-three',
    name: 'RandomFish',
    avatar: 'randomfish-profile.png',
    description: 'Randomly chooses among Stockfish’s top three moves.',
    videoId: 'qJLOLAKx6Fo',
    videoTitle: 'Stockfish, But It Randomly Picks 1 of 3 Top Moves',
    movePolicy: { type: 'random-top-n', count: 3 },
  }),
  videoVariant({
    id: 'iwc-zero-evaluation',
    name: 'DrawFish',
    avatar: 'drawfish-profile.png',
    description: 'Chooses the move that keeps the evaluation closest to 0.00.',
    videoId: 'mUmTeprNjzU',
    videoTitle: 'Stockfish, But It Plays for 0.00 Evaluation',
    movePolicy: { type: 'target-evaluation', targetCp: 0 },
  }),
  videoVariant({
    id: 'iwc-second-best',
    name: 'BetaFish',
    avatar: 'betafish-profile.png',
    description: 'Always plays Stockfish’s second-best move.',
    videoId: 'TQR5zCSzrqw',
    videoTitle: 'Stockfish, But It Plays the 2nd Best Move...',
    movePolicy: { type: 'ranked-move', rank: 2 },
  }),
  videoVariant({
    id: 'iwc-hungry-martin',
    name: 'HungryMartin',
    rating: 250,
    avatar: 'hungrymartin-bearded-profile.png',
    avatarScale: 1,
    description: 'Starts at 250 and gains 1000 Elo after a capture or check.',
    videoId: '0gnWnkyg6VA',
    videoTitle: 'Martin, But He Gains 1000 ELO If He Captures or Checks',
    initialElo: 250,
    trigger: 'own-capture-or-check',
    eloDelta: 1000,
    movePolicy: { type: 'rating-strength' },
  }),
  videoVariant({
    id: 'iwc-capture-toggle',
    name: 'Stockfish',
    rating: 3600,
    category: 'stockfish',
    avatar: 'capture-toggle-stockfish-profile.png',
    avatarStates: {
      stockfish: 'capture-toggle-stockfish-profile.png',
      martin: 'martin-profile.png',
    },
    identityStates: {
      stockfish: {
        name: 'Stockfish',
        displayRating: 3600,
        country: 'United States',
        countryCode: 'us',
      },
      martin: {
        name: 'Martin',
        displayRating: 250,
        country: 'Bulgaria',
        countryCode: 'bg',
      },
    },
    description: 'Starts as Stockfish at 3600. Each capture it makes hands the next move to Martin at 250, then back again.',
    videoId: 'Q6sj5N3oQjI',
    videoTitle: 'Stockfish, But It Alternates 250 ELO to 3600 ELO Every Capture',
    initialElo: 3600,
    minElo: 250,
    maxElo: 3600,
    trigger: 'own-capture',
    movePolicy: {
      type: 'capture-toggle',
      stockfishElo: 3600,
      martinElo: 250,
    },
    capabilities: { dynamicAvatar: true },
  }),
  videoVariant({
    id: 'iwc-worstfish',
    name: 'WorstFish',
    country: '',
    countryCode: '',
    avatar: 'worstfish-profile.png',
    description: 'Searches every legal move and deliberately chooses the worst.',
    videoId: 'SsOtn3iDkv8',
    videoTitle: 'I Played Against The WORST Chess Bot',
    movePolicy: { type: 'worst-move', allLegalMoves: true },
  }),
  videoVariant({
    id: 'iwc-martinfish',
    name: 'Martinfish',
    avatar: 'martinfish-profile.png',
    description: 'Alternates one Stockfish move with one Martin move.',
    videoId: 'DKicZI66j6k',
    videoTitle: 'I Combined Stockfish and Martin',
    initialElo: null,
    movePolicy: { type: 'cycle', stockfishMoves: 1, martinMoves: 1 },
  }),
  videoVariant({
    id: 'iwc-martinfish-2',
    name: 'Martinfish 2.0',
    avatar: 'martinfish-profile.png',
    description: 'Plays two Stockfish moves, then one Martin move.',
    videoId: '2L2yE15BdMg',
    videoTitle: 'Martinfish 2.0',
    initialElo: null,
    movePolicy: { type: 'cycle', stockfishMoves: 2, martinMoves: 1 },
  }),
  videoVariant({
    id: 'iwc-martinfish-3',
    name: 'Martinfish 3.0',
    avatar: 'martinfish-profile.png',
    description: 'Plays three Stockfish moves, then one Martin move.',
    videoId: 'PSSEOUxNTrE',
    videoTitle: 'Martinfish 3.0',
    initialElo: null,
    movePolicy: { type: 'cycle', stockfishMoves: 3, martinMoves: 1 },
  }),
  videoVariant({
    id: 'iwc-random-martinfish',
    name: 'Random Martinfish',
    avatar: 'martinfish-profile.png',
    description: 'Uses Stockfish on 90% of moves and Martin on the other 10%.',
    videoId: 'Y0LOmrRicgw',
    videoTitle: 'Martinfish Just Got Worse',
    initialElo: null,
    movePolicy: { type: 'random-hybrid', stockfishChance: 0.9 },
  }),
  videoVariant({
    id: 'iwc-martinfish-80-20',
    name: 'Martinfish 80/20',
    avatar: 'martinfish-profile.png',
    description: 'Uses Stockfish on 80% of moves and Martin on the other 20%.',
    videoId: 'ew6NU1Z_G4k',
    videoTitle: 'Martinfish is BACK',
    initialElo: null,
    movePolicy: { type: 'random-hybrid', stockfishChance: 0.8 },
  }),
  videoVariant({
    id: 'iwc-martinfish-95-5',
    name: 'Martinfish 95/5',
    avatar: 'martinfish-profile.png',
    description: 'Uses Stockfish on 95% of moves and Martin on the other 5%.',
    videoId: 'MwXcULBxA_s',
    videoTitle: 'Can I BEAT Martinfish?',
    initialElo: null,
    movePolicy: { type: 'random-hybrid', stockfishChance: 0.95 },
  }),
  videoVariant({
    id: 'iwc-evil-martin',
    name: 'Evil Martin',
    rating: 250,
    avatar: 'sleepy-martin-profile.png',
    avatarStates: {
      sleepy: 'sleepy-martin-profile.png',
      evil: 'evil-martin-profile.png',
    },
    description: 'Sleeps at 250 while safe, then wakes up at 3000 when the position turns against him.',
    videoId: 'LfzNhUjl3qE',
    videoTitle: 'Martin Becomes Evil When He Starts Losing',
    initialElo: 250,
    maxElo: 3000,
    movePolicy: {
      type: 'evil-martin',
      awakeElo: 3000,
      sleepyElo: 250,
      wakeThresholdCp: -600,
      permanentWake: true,
    },
    capabilities: { dynamicAvatar: true },
  }),
  videoVariant({
    id: 'iwc-evil-martin-2',
    name: 'Evil Martin 2.0',
    rating: 250,
    avatar: 'sleepy-martin-profile.png',
    avatarStates: {
      sleepy: 'sleepy-martin-profile.png',
      evil: 'evil-martin-profile.png',
    },
    description: 'Sleeps at 250, then permanently wakes up at 3000 after falling five points behind.',
    videoId: 'hymgaszLxnw',
    videoTitle: 'Evil Martin Just Got WORSE',
    initialElo: 250,
    maxElo: 3000,
    movePolicy: {
      type: 'evil-martin',
      awakeElo: 3000,
      sleepyElo: 250,
      wakeThresholdCp: -500,
      permanentWake: true,
    },
    capabilities: { dynamicAvatar: true },
  }),
])

export const IWANTCHECKMATE_VARIANT_IDS = Object.freeze(
  IWANTCHECKMATE_VIDEO_PROFILES.map((profile) => profile.id),
)

export function getIWantCheckmateProfile(profileId) {
  const resolvedId = profileId === 'martinfish' ? 'iwc-smartin' : profileId
  return IWANTCHECKMATE_VIDEO_PROFILES.find((profile) => profile.id === resolvedId) || null
}

export function isIWantCheckmateProfile(profile) {
  return Boolean(profile?.capabilities?.videoVariant)
}
