...
test('Trixize and Akshit play in Bot vs Bot mode without dialogue', async ({ page }) => {
  ...
  await expect(page.getByLabel('Bot conversation')).toHaveCount(0)
...
test('reduced motion keeps dialogue disabled and does not delay the game', async ({ page }) => {
...
test('Trixize opens with Nf3 without dialogue', async ({ page }) => {
  await page.getByRole('button', { name: /Trixize/ }).click()
  await page.getByRole('button', { name: 'Black', exact: true }).click()
  await page.getByRole('button', { name: 'Play', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Nf3', exact: true })).toBeVisible({ timeout: 12000 })
