...
void UCIEngine::loop() {
    ...
    do
    {
        ...
        else if (token == "bmreport")
        {
            std::string fen;
            std::getline(is >> std::ws, fen);
            if (fen.empty())
                print_info_string(BadManners::report(engine.position(), engine.get_options()));
            else
            {
                StateInfo reportState;
                Position  reportPos;
                reportPos.set(fen, engine.get_options()["UCI_Chess960"], &reportState);
                print_info_string(BadManners::report(reportPos, engine.get_options()));
            }
        }
        else if (token == "eval")
            engine.trace_eval();
