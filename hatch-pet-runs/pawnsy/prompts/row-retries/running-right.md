Create Codex pet row `running-right` for `pawnsy`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact whole-body talking black chess pawn mascot: glossy obsidian pawn body, simple carved expressive face with bright eyes and small mouth, tiny mitten-like hands, confident friendly strategist personality, no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
