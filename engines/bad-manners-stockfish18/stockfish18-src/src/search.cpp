...
void Search::Worker::start_searching() {
    ...
    BadManners::reorder_root_moves(rootPos, bestThread->rootMoves, options);
    ...
    if (BadManners::enabled(options) && bool(options["VerboseChallengeOutput"]))
        sync_cout << "info string "
                  << BadManners::explain_selected_move(rootPos, bestThread->rootMoves[0], options)
                  << sync_endl;
...
void Search::Worker::iterative_deepening() {
    ...
    size_t multiPV = BadManners::effective_multipv(options, rootMoves.size());
